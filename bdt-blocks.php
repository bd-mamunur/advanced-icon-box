...
final class BDT_ICON_BOX_BLOCK_CLASS {
    ...
    public function define_constants(){
        define('BDT_NAME', 'advanced-icon-box');
		define('BDT_SLUG', 'advanced-icon-box');
    ...
    public function check_php_version(){
        $message = sprintf(
            esc_html__( 'BDT Blocks requires minimum PHP version %s where your current PHP version is %2s. Please update your PHP version to enable BDT Blocks features. ', 'advanced-icon-box' ),
            '5.6',
            ZOLO_PHP_VERSION
        );
    ...
    public function check_wp_version(){
        $message = sprintf(
            esc_html__( 'BDT Blocks requires minimum WordPress version %s where your current WordPress version is %2s. Please update your WordPress version to enable BDT Blocks features. ', 'advanced-icon-box' ),
            '5.8',
            ZOLO_WP_VERSION
        );
        $html_message = sprintf( '<div class="error">%s</div>', wpautop( $message ) );
    ...
    public function gutenberg_unavailable_notice(){
        ...
        $message = sprintf(
            __( 'The <%1$s>%2$s</%1$s> plugin requires <%1$s>Gutenberg</%1$s> plugin installed & activated.', 'advanced-icon-box' ),
            $tag = 'strong',
            ZOLO_NAME
        );

        $action_url = wp_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin=gutenberg' ), 'install-plugin_gutenberg' );
        $button_label = __( 'Install Gutenberg', 'advanced-icon-box' );
 ...
 * Kickoff
*/
function bdt_icon_box_block(){
	return BDT_ICON_BOX_BLOCK_CLASS::instance();
}
